...
import { FaUser, FaMapMarkerAlt, FaArrowLeft, FaCreditCard, FaLocationArrow, FaShoppingBag, FaPlus, FaStore, FaCheckCircle, FaTruck, FaPhoneAlt, FaClock, FaInfoCircle } from "react-icons/fa";
...
export default function Checkout() {
  ...
  const { cartItems, selectedBranch, setSelectedBranch, fulfillmentType, setFulfillmentType } = useCart();
  ...
  // Compute delivery range for the selected address + branch (ONLY for Home Delivery)
  const selectedAddressObj = !isPickup ? (addresses.find(a => a.id === selectedAddress) || null) : null;
  ...
  const handleProceedToBilling = () => {
    ...
    const checkoutState = {
      ...
      orderType: isPickup ? "pickup" : "homeDelivery",
      selectedAddress: isPickup ? null : selectedAddressObj,
      total,
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="card-premium p-6 sm:p-8">
              ...
              {/* Distance validation feedback (Only for Home Delivery) */}
              {!isPickup && deliveryValidation && (
            ...
            <div className="card-premium p-6 sm:p-8">
              <h3 className="font-serif text-xl font-bold text-[#2B1D17] mb-4 flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-[#F47A20]/10 flex items-center justify-center">
                  <FaTruck className="text-[#F47A20]" />
                </div>
                Fulfillment Method
              </h3>
              <div className="grid grid-cols-2 gap-3 p-1.5 bg-gray-100/80 rounded-2xl">
                <button
                  ...
                  className={`py-3.5 px-4 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2.5 ${
                    !isPickup
                      ? "bg-[#F47A20] text-white shadow-md shadow-[#F47A20]/20"
                      : "text-gray-600 hover:text-gray-900 hover:bg-gray-200/50"
                  }`}
                >
                  <FaTruck size={16} /> Home Delivery
                </button>
                <button
                  type="button"
                ...
                >
                  <FaShoppingBag size={16} /> Store Pickup
                </button>
              </div>
            </div>
            ...
            {/* Address (If Delivery) OR Pickup Details (If Pickup) */}
            {isPickup ? (
              <div className="card-premium p-6 sm:p-8">
                <h3 className="font-serif text-xl font-bold text-[#2B1D17] mb-4 flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-[#F47A20]/10 flex items-center justify-center"><FaStore className="text-[#F47A20]" /></div>
                ...
                {selectedBranch ? (
                  <div className="p-5 rounded-2xl bg-[#FFF5EB] border border-[#F47A20]/20 space-y-3">
                    <div className="flex items-center gap-2">
                      <span className="text-[#F47A20] text-xs bg-[#F47A20]/10 px-2.5 py-1 rounded-full font-bold uppercase tracking-wider">
                        🛍️ Pickup Selected
                      </span>
                    </div>
                    <div>
                      <h4 className="font-serif font-bold text-[#2B1D17] text-lg">
                        {formatBranchName(selectedBranch)}
                      </h4>
                      <p className="text-[#2B1D17]/70 text-sm mt-1">
                        {selectedBranch.fullAddress || selectedBranch.addressLine || selectedBranch.city}
                      </p>
                      {selectedBranch.phone && (
                        <p className="text-[#2B1D17]/60 text-xs mt-1.5 flex items-center gap-1.5">
                          <FaPhoneAlt className="text-[#F47A20]" size={12} /> {selectedBranch.phone}
                        </p>
                      )}
                    </div>
                    <div className="pt-3 border-t border-[#F47A20]/15 flex items-center gap-2 text-xs text-[#2B1D17]/70">
                      <FaClock className="text-[#F47A20]" size={14} />
                      <span>Estimated Ready Time: <strong>15–25 minutes</strong> after order confirmation</span>
                    </div>
                  </div>
                ) : (
                  <div className="p-5 rounded-2xl bg-amber-50 border border-amber-200 text-amber-800 text-sm flex items-center gap-2">
                    <FaInfoCircle className="text-amber-500 flex-shrink-0" />
                    <span>Please select a branch above to view pickup location details.</span>
                  </div>
                )}
              </div>
            ) : (
            ...
            <div className="card-premium p-6 sticky top-24">
              ...
              {selectedBranch && (
                <div className="flex items-center gap-2 mb-4 p-3 bg-[#FFF5EB] rounded-xl border border-[#F47A20]/20">
                  <FaStore className="text-[#F47A20] text-sm flex-shrink-0" />
                  <span className="text-xs font-medium text-[#2B1D17]">Branch: <b className="text-[#F47A20]">{formatBranchName(selectedBranch)}</b></span>
                </div>
              )}
              <div className="flex items-center justify-between mb-4 p-3 bg-gray-50 rounded-xl text-xs font-medium">
                <span className="text-gray-500">Method:</span>
                <span className="text-[#F47A20] font-bold flex items-center gap-1">
                  {isPickup ? <><FaShoppingBag size={12} /> Store Pickup</> : <><FaTruck size={12} /> Home Delivery</>}
                </span>
              </div>
              ...
              <div className="border-t border-gray-100 pt-4 space-y-2">
                ...
                <div className="flex justify-between text-sm"><span className="text-[#2B1D17]/60">Delivery</span><span className="text-green-600 font-medium">{isPickup ? "N/A (Pickup)" : "Free"}</span></div>
              ...
              {!canProceed() && (
                <p className="text-xs text-center text-[#2B1D17]/40 mt-2">
                  {!userInfo.fullName || !userInfo.email || !userInfo.phone
                    ? "Complete your user info to continue"
                    : !selectedBranch

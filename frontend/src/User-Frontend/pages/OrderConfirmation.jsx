    ...
    const loadOrderData = async () => {
      ...
      if (passedOrderId) {
        try {
          ...
          const isPickup = order.orderType === 'pickup' || df === 0 || !order.deliveryLocation;
          const tot = typeof order.totalAmount === "number" ? order.totalAmount : +(sub + tax + (isPickup ? 0 : df)).toFixed(2);
          setOrderData({
            ...
            items, subtotal: sub, tax, deliveryFee: isPickup ? 0 : df, total: tot, isPickup,
          });
          emptyCart();
